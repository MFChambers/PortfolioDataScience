...
# Summary of overall project work
    ...
    * Performance: ~78–80% test accuracy; Random Forest offered the best balance of robustness and interpretability (with ranked feature importances).
    * Feature importance: Living area was most predictive, followed by bathrooms and stories—consistent with domain intuition.
...
Additional Analysis: 
    ...
    * Regression stretch: Predicted yrbuilt with a Random Forest Regressor (strong 𝑅2 and reasonable RMSE), useful for estimating missing build years.
...
# Prepare data for visuals (safe for plotting, not for modeling)
df['stories_str'] = df['stories'].astype(str)
df['numbaths_grouped'] = pd.cut(df['numbaths'], bins=[0, 1, 2, 3, 4, np.inf],
                                labels=['<=1', '1-2', '2-3', '3-4', '4+'])
df['livearea_grouped'] = pd.cut(df['livearea'],
                                bins=[0, 1000, 1500, 2000, 2500, np.inf],
                                labels=['<1000', '1000-1500', '1500-2000', '2000-2500', '2500+'])

# Labels for plots only
...
```{python}
# ---- Features / Target (numeric-only, no leakage) ----
# Target
target = df["before1980"].astype(int)

# Columns that would leak the answer or are identifiers
leak_or_id = {"before1980", "yrbuilt", "parcel"}

# Keep only numeric columns, drop leakage/id columns
numeric_cols = df.select_dtypes(include=[np.number]).columns
use_cols = [c for c in numeric_cols if c not in leak_or_id]

features = df[use_cols].copy().fillna(0)
